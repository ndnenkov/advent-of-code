...
function run40Times(polymer: string, rules: {[key: string]: string}): {[key: string]: number} {
  let counts = initialCounts(polymer)

  for (let stepNumber = 1; stepNumber <= 40; stepNumber++) {
...
console.log(quantify(run40Times(template, rules)))
