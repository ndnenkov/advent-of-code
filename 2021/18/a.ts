...
function findSumMagnitude(snailfishNumbers: string[]): number {
  const sum = snailfishNumbers
    .map(parseSnailfishNumber)
    .reduce((sum, numbers) => addSnailfishNumbers(sum, numbers))

  return magnitude(sum)
}

console.log(findSumMagnitude(homework))
