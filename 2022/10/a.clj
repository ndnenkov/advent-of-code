...
(defn run
  ([instructions] (run instructions 1 [0 1]))
  ([instructions x xes]
       ...
       (let [remaining-instructions (rest instructions)
         ...
         (case operation
           ...
           "addx" (let [new-x (+ x value)]
                    (recur remaining-instructions new-x (into xes [x new-x]))))))))

(def x-values (run instructions))
(def interesting-steps [20 60 100 140 180 220])
(def interesting-signals-strength (->> interesting-steps
                                       (map x-values)
                                       (#(map vector % interesting-steps))
                                       (map (fn [[x step]] (* x step)))
                                       (reduce +)))

(pr interesting-signals-strength)
