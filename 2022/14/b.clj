...
(defn pour
  ...
  ([cave [x y]]
   (let [below [x (inc y)]
         left [(dec x) (inc y)]
         right [(inc x) (inc y)]
         cave-with-particle (assoc cave [x y] "o")]
     (cond
       (= y (dec rock-bottom)) (recur cave-with-particle sand-origin)
       (not (cave below)) (recur cave below)
       (not (cave left)) (recur cave left)
       (not (cave right)) (recur cave right)
       (not= [x y] sand-origin) (recur cave-with-particle sand-origin)
       :else cave-with-particle))))
