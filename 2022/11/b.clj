...
(def worry-levels-lcm (->> monkeys (map :test) (reduce num/lcm)))
...
(defn throwoff [monkeys turn]
  (if (empty? ((nth monkeys turn) :items)) monkeys
      (let [player (nth monkeys turn)
            ...
            after-operation (mod ((player :operation) item) worry-levels-lcm)
            test-passed (= 0 (mod after-operation (player :test)))
            ...
            updated-pass-to (update pass-to :items (fn [items] (conj (vec items) after-operation)))]
        (recur
         (-> monkeys
             vec
...
(defn play
  ...
  ([monkeys round turn]
   (let [last-turn (->> monkeys count dec (= turn))
     ...
     (if (= 10001 next-round) next-monkeys
